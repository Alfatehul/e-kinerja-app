...
export default async function TambahTorPage() {
  const supabase = await createClient(); const { data: proposals } = await supabase.from("budget_proposals").select("id,number,program,kegiatan,uraian,volume,satuan,harga_satuan,sumber_dana").order("created_at", { ascending: false });
